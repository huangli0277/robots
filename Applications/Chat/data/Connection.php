...
class Connection
{
	...
	function get_hui() {
		...
		curl_setopt($ch, CURLOPT_URL, 'https://gate.io/json_svr/query_push/?type=push_main_rates&symbol=USDT_CNY&c'.rand(100000,999999));
...
//		if ($res === false) throw new Exception('Could not get reply: '.curl_error($ch));
	...
	function get_pris() {
		...
		curl_setopt($ch, CURLOPT_URL, 'http://data.gate.io/api2/1/tickers?c='.rand(100000,999999));
		curl_setopt($ch, CURLOPT_SSL_VERIFYPEER, FALSE);
		// run the query
		$res = curl_exec($ch);
//		if ($res === false) throw new Exception('Could not get reply: '.curl_error($ch));
